		...
		do
			...
			do
				...
				for h in ${server_machines[@]}; do
					cmd=$CMD_SRV" -startSequencer -siteId "${REGION_NAME[$((ri))]}" -master "${REGION_NAME[0]}" -sequencers "$sequencers" "$m
					echo "Start Sequencer "$h "CMD" $cmd
					ssh $USERNAME@$h "nohup "$cmd " 2>&1 | tee dc_sequencer_console.log" &
					sleep 2
					cmd=$CMD_SRV" -startServer -siteId "${REGION_NAME[$((ri))]}" -master "${REGION_NAME[0]}" -servers "${servers[@]}" "$m
				...
				for h in ${client_machines[@]}; do
					cmd=$makeDir" ; "$CMD_CLT" -run -siteId "${REGION_NAME[$((ri))]}" -master "${REGION_NAME[0]}" -config "$k" -threads "$j" -srvAddress "${indigos[$((ri))]}" -results_dir "$OUTPUT_DIR" -shepard "$SHEPARD_URL" "$m
					ri=`expr $ri + 1`
...
#					ssh $USERNAME@$h "nohup "$cmd" > client_console.log" &
				...
				for h in ${client_machines[@]}; do
					...
					awk="awk -F '\t'  '{print \$2\" \"\$4}' "$OUTPUT_DIR"tournaament_results_"${REGION_NAME[$((ri))]}".log"
