	...
	do
		...
		do
			...
			do
				...
				OUTPUT_DIR=$INDIGO_ROOT"long_results_tournament-strong-c-"$k"-r"$i"-t"$j"/"
				...
				for h in ${server_machines[@]}; do
					...
					cmd=$CMD_SRV" -startServer -siteId STRONG -master STRONG -sequencerUrl "${SEQUENCERS[0]}" -servers "${s[@]}" "$m" -redblue "${SEQUENCERS[0]}
