...
public class LWWRegisterCRDT<V> extends BaseCRDT<LWWRegisterCRDT<V>> {
	private V val;
	...
	private LWWRegisterCRDT(CRDTIdentifier id, TxnHandle txn, CausalityClock clock, V val, long timestamp, TripleTimestamp timestampTiebreaker) {
		super(id, txn, clock);
		this.val = val;
	...
	protected void applySet(final long timestamp, final TripleTimestamp tripleTimestamp, final V value) {
		if (timestamp < this.timestamp || (timestamp == this.timestamp && tripleTimestamp.compareTo(this.timestampTiebreaker) <= 0)) {
			// Last Writer Wins
