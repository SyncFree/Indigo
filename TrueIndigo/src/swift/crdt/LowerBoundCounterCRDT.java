...
public class LowerBoundCounterCRDT extends BoundedCounterCRDT<LowerBoundCounterCRDT> {
	...
	public LowerBoundCounterCRDT(CRDTIdentifier id, TxnHandle txn, CausalityClock clock, int initVal, Map<String, Map<String, Integer>> permissions, Map<String, Integer> decrements) {
	...
	public boolean increment(int amount, String siteId) {
		BoundedCounterIncrement<LowerBoundCounterCRDT> update = new BoundedCounterIncrement<LowerBoundCounterCRDT>(siteId, amount);
		applyInc(update);
	...
	public boolean decrement(int amount, String siteId) {
		if (availableSiteId(siteId) >= amount) {
			BoundedCounterDecrement<LowerBoundCounterCRDT> update = new BoundedCounterDecrement<LowerBoundCounterCRDT>(siteId, amount);
			applyDec(update);
			registerLocalOperation(update);
			return true;
		}
	...
	public boolean transfer(int amount, String originId, String targetId) {
		if (availableSiteId(originId) >= amount) {
			BoundedCounterTransfer<LowerBoundCounterCRDT> update = new BoundedCounterTransfer<LowerBoundCounterCRDT>(originId, targetId, amount);
