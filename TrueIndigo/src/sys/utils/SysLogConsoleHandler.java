...
public class SysLogConsoleHandler extends ConsoleHandler {
	...
	static String HOSTNAME = IP.localHostname();
		...
		private static String callerClass(LogRecord r) {
			...
			for (int i = stack.length; --i >= 0;)
				if (name.equals(stack[i].getClassName()))
					return String.format("[%5s : %4d : %.4f]", name, stack[i].getLineNumber(), System.currentTimeMillis() / 1000.0);
			return "[]";
		}

		@Override
		public String format(LogRecord r) {
			final int TEXT_WIDTH = 160;
			final String padding = "                                               ";

			StringBuilder res = new StringBuilder(HOSTNAME).append(" @ ").append(r.getLevel()).append(" : ").append(r.getMessage());
