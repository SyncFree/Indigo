 ...
 *****************************************************************************/
package indigo.application.tournament.invrepair;

import static sys.Context.Networking;
import indigo.application.tournament.reservations.Match;
import indigo.application.tournament.reservations.NamingScheme;
import indigo.application.tournament.reservations.Player;
...
public class TournamentServiceOps {
	...
	private final Service fakeStub;
	...
	protected String selectPlayer(int site) throws SwiftException {
		stub.beginTxn();
		AddWinsSetCRDT<String> playerIndex = stub.get(NamingScheme.forPlayerIndex(site + ""), true,
	...
	@SuppressWarnings("unchecked")
	protected void addNewPlayersToTournament(final String[] playerNames, final String tournamentName)
		...
		for (String playerName : playerNames) {
			...
			AddWinsSetCRDT<String> tournament = stub.get(NamingScheme.forTournament(tournamentName), true,
					AddWinsSetCRDT.class);
	...
	protected boolean addTournament(final int tournamentSite, final String tournamentName, final int maxSize)
			throws SwiftException {
		boolean result;
		stub.beginTxn();
		result = _addTournament(tournamentSite, tournamentName);
		stub.endTxn();
		if (fakeStub != null)
			fakeStub.request(surrogate, new CurrentClockRequest());
		return result;
	...
	@SuppressWarnings("unchecked")
	private boolean _addTournament(final int site, final String tournamentName) throws SwiftException {
		boolean result = true;
		try {
	...
	protected boolean removeTournament(final int site, final String tournamentName) throws SwiftException {
		...
		try {
			...
			AddWinsSetCRDT<String> tournamentIndex = stub.get(NamingScheme.forTournamentIndex(site + ""), false,
					AddWinsSetCRDT.class);
	...
	@SuppressWarnings("unchecked")
	protected boolean _enrollTournament(final int site, final String playerName, final String tournamentName)
			throws SwiftException {
		boolean result = true;
		...
		try {
			...
			// Check tournament exists
			// TODO: If the tournament does not exist, the operation shall fail.
			// Only add it again if it exists.
			// Check other transformations.
			stub.get(NamingScheme.forTournamentIndex(site + ""), false, AddWinsSetCRDT.class);
			_addTournament(site, tournamentName);
	...
	protected boolean enrollTournament(final int site, final String playerName, final String tournamentName)
			throws SwiftException {
		boolean result = true;
		stub.beginTxn();
		try {
			_enrollTournament(site, playerName, tournamentName);
		} catch (SwiftException e) {
		} finally {
			stub.endTxn();
			if (fakeStub != null)
	...
	@SuppressWarnings("unchecked")
	protected boolean doMatch(String matchId, String tournamentName, String player1, String player2)
			throws SwiftException {
		boolean result = true;
		try {
			stub.beginTxn();
			...
			if (player1Tournaments.lookup(tournamentName) && player2Tournaments.lookup(tournamentName)) {
				AddWinsSetCRDT<Match> matchHistory = stub.get(NamingScheme.forMatchHistory(tournamentName), true,
						AddWinsSetCRDT.class);
				Match match = new Match(matchId, player1, player2);
				int site = Integer.parseInt(tournamentName.split("_")[0]);
				_enrollTournament(site, player1, tournamentName);
				_enrollTournament(site, player2, tournamentName);
