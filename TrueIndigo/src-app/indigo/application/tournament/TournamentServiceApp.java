    ...
    public void initTournaments(Indigo stub, final List<String> commands, AtomicInteger counter, int total,
        ...
        try {
            ...
            for (String line : commands) {
                ...
                AddWinsSetCRDT<String> tournamentSet = client._addTournament(tournamentSite, tournament[1], maxPlayers);

                for (int i = 1; i < toks.length; i++) {
                    String[] playerToks = toks[i].split("_");
                    int playerSite = Integer.parseInt(playerToks[0]);
                    client._addNewPlayerToTournament(playerToks[1], playerSite, tournament[1], tournamentSet);
                }
            }
            stub.endTxn();
        } catch (Exception e1) {
